...
def parse_price(iid, price, sellerid):
    ...
    """
    data = get_html(
        'http://ajax.tbcdn.cn/json/umpStock.htm?itemId=%s&p=1&rcid=1&price=%s&sellerId=%s' % (iid, price, sellerid),
        referer='http://item.taobao.com/item.htm?id=%s' % iid, verbose=False)
...
def parse_quantity(iid, sellerid, qmd5):
    ...
    提取货物销量
    """
    url = "http://ajax.tbcdn.cn/json/ifq.htm?id=%s&sid=%s&p=1&ap=0&ss=0&free=0&q=1&ex=0&exs=0&at=b&ct=0&sbn=%s" % (
    iid, sellerid, qmd5)
    data = get_html(url)
...
def getTmallItemInfo(iid, keyword=''):
    ...
    data = get_html(url, referer="http://detail.tmall.com/item.htm?id=%s" % iid).decode(
        'gbk')  # .replace('\r\n','').replace('\t','')
